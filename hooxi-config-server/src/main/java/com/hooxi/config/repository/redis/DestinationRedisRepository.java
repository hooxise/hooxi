...
public class DestinationRedisRepository {
    ...
    @Autowired
    public DestinationRedisRepository(ReactiveRedisOperations<String, DestinationCacheEntity> template,
    ...
    public Mono<List<DestinationCacheEntity>> findDestinations(FindDestinationsRequest req) {
        Flux<List> f = template.execute(findDestinationScript, new ArrayList<>(),
            Arrays.asList(req.getTenantId(), req.getDomain(), req.getSubDomain(), req.getEventType()), argsWriter,
            resultReader);
        return f.collect(ArrayList<DestinationCacheEntity>::new, (containerLst, lst) -> containerLst.addAll(lst))
            .map(lstResult -> {
