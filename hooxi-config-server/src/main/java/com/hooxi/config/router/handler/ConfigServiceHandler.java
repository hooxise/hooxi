...
public class ConfigServiceHandler {
  ...
  public Mono<ServerResponse> findDestinationMappings(ServerRequest serverRequest) {
    ...
    MultiValueMap<String, String> queryParamsMap = serverRequest.queryParams();
    ...
    FindDestinationsRequest req =
        FindDestinationsRequestBuilder.aFindDestinationsRequest()
            .withDomain(Optional.ofNullable(queryParamsMap.getFirst("domainId")).orElse(""))
            .withTenantId(Optional.ofNullable(pathVariables.get("tenantId")).orElse(""))
            .withSubDomain(Optional.ofNullable(queryParamsMap.getFirst("subdomainId")).orElse(""))
            .withEventType(Optional.ofNullable(queryParamsMap.getFirst("eventType")).orElse(""))
            .build();
    Mono<FindDestinationsResponse> fdr = configService.findDestinations(Mono.just(req));
    return ServerResponse.ok()
        .contentType(MediaType.APPLICATION_JSON)
        .body(fdr, FindDestinationsResponse.class)
        .onErrorResume(
            e -> {
              logger.error("error in finding destinations for %s ", serverRequest, e);
              return Mono.just(
                      String.format("error in finding destinations for %s ", serverRequest))
                  .flatMap(
                      s ->
  ...
  @Operation(
      description = "Add Destination Mappings",
      requestBody =
          @RequestBody(
              content =
                  @Content(schema = @Schema(implementation = AddDestinationMappingRequest.class))),
  ...
  public Mono<ServerResponse> addDestinationMapping(ServerRequest serverRequest) {
    return configService
        .addDestinationMapping(
            serverRequest.bodyToMono(AddDestinationMappingRequest.class),
            serverRequest.pathVariable("tenantId"))
        .flatMap(dt -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).bodyValue(dt));
  }

  @Operation(
      description = "Get Destination by tenantId and destinationId",
  ...
  public Mono<ServerResponse> addDestination(ServerRequest serverRequest) {
    return configService
        .addDestination(
            serverRequest.bodyToMono(AddDestinationRequest.class),
            serverRequest.pathVariable("tenantId"))
        .flatMap(dt -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).bodyValue(dt));
  }

  @Operation(
      description = "Get all destinations for tenant",
  ...
  public Mono<ServerResponse> findDestinationsForTenant(ServerRequest serverRequest) {
    return ServerResponse.ok()
        .contentType(MediaType.APPLICATION_JSON)
        .body(
            configService.findDestinationForTenants(serverRequest.pathVariable("tenantId")),
            DestinationResponse.class);
  }

  @Operation(
      description = "Delete Destination Mappings",
  ...
  public Mono<ServerResponse> deleteDestinationMapping(ServerRequest serverRequest) {
    return configService
        .deleteDestinationMapping(serverRequest.pathVariable("destMappingId"))
        .flatMap(
            dstM ->
                ServerResponse.ok()
                    .contentType(MediaType.APPLICATION_JSON)
                    .bodyValue(
                        DeleteDestinationMappingResponseBuilder.aDeleteDestinationMappingResponse()
                            .withMessage("Delete successful")
                            .withDeletedMapping(
                                DestinationMappingResponseBuilder.aDestinationMappingResponse()
                                    .withDestinationMappingId(dstM.getDestinationMappingId())
                                    .withEventType(dstM.getEventType())
                                    .withTenantId(dstM.getTenantId())
                                    .withDomainId(dstM.getDomainId())
                                    .withSubdomainId(dstM.getSubdomainId())
                                    .withDestinationInfo(
                                        DestinationResponseBuilder.aDestinationResponse()
                                            .withDestination(dstM.getDestination())
                                            .withDestinationId(dstM.getDestinationId())
                                            .withTenantId(dstM.getTenantId())
                                            .build())
                                    .withStatus(dstM.getStatus().toString())
                                    .build())
                            .build()))
        .switchIfEmpty(ServerResponse.notFound().build())
        .onErrorResume(
            e -> {
              logger.error(
                  "error in processing delete destination mapping request for destinationMappingId {}",
                  serverRequest.pathVariable("destMappingId"),
                  e);
