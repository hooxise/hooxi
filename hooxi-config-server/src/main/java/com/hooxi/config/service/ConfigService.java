...
public class ConfigService {
    ...
    @Autowired
    public ConfigService(DestinationRedisRepository destinationRedisRepository,
    ...
    public Mono<FindDestinationsResponse> findDestinations(Mono<FindDestinationsRequest> req) {
        return req.flatMap((fdr) -> destinationRedisRepository.findDestinations(fdr)
            .onErrorResume(CacheMissException.class, (ce) -> findDestinationsAndUpdateCache(fdr).collectList())
            .map(lstDstnCacheEntity -> FindDestinationsResponseBuilder.aFindDestinationsResponse()
                .withDestinationMappings(lstDstnCacheEntity.stream()
                    .map(dstnCacheEnt -> DestinationMappingResponseBuilder.aDestinationMappingResponse()
                        ...
                        .withDestinationMappingId(dstnCacheEnt.getDestinationMappingId())
                        .withDestinationInfo(DestinationResponseBuilder.aDestinationResponse()
                            .withTenantId(dstnCacheEnt.getTenantId())
                            .withDestinationId(dstnCacheEnt.getDestinationId())
                            .withDestination(dstnCacheEnt.getDestination())
                            .build())
                        .withStatus(dstnCacheEnt.getStatus() == null ? "" :
    ...
    private Flux<DestinationCacheEntity> findDestinationsAndUpdateCache(FindDestinationsRequest req) {
        logger.debug("searching in database");
        return destinationMappingRepository.findDestinationConfigFor(req.getTenantId(), req.getDomain(),
                req.getSubDomain(), req.getEventType())
            .map(dstConfig -> DestinationCacheEntityBuilder.aDestinationCacheEntity()
            ...
            .flatMap(de -> {
                ...
                return destinationRedisRepository.addDestinationConfigToCache(key, de)
                    .map(l -> de)
                    .onErrorContinue((err, dceForErr) -> logger.error(
                        "error in storing DestinationCacheEntity {} in cache", dceForErr, err));
            });
    }

    public Mono<DestinationResponse> addDestination(Mono<AddDestinationRequest> dest, String tenantId) {
        return dest.flatMap(dstReq -> {
            ...
            return destinationRepository.save(de).map(savedDe -> {
    ...
    public Mono<DestinationResponse> findDestinationById(Long destinationId, String tenantId) {
        return destinationRepository.findByIdAndTenantId(destinationId, tenantId)
            .map(de -> DestinationResponseBuilder.aDestinationResponse().withDestinationId(de.getId())
                .withTenantId(de.getTenantId()).withDestination(de.getDestination()).build());
    }

    public Flux<DestinationResponse> findDestinationForTenants(String teanntId) {
        return destinationRepository.findByTenantId(teanntId)
            .map(de -> DestinationResponseBuilder.aDestinationResponse().withDestinationId(de.getId())
                .withTenantId(de.getTenantId()).withDestination(de.getDestination()).build());
    }

    public Mono<DestinationMappingResponse> addDestinationMapping(Mono<AddDestinationMappingRequest> dest,
                                                                  String tenantId) {
        return dest.flatMap(dstReq -> {
            ...
            return destinationMappingRepository.save(de).map(savedDe -> {
                ...
                return dstResp;
            }).flatMap(dstResp2 -> destinationRepository.findById(dstReq.getDestinationId()).map(dstn -> {
                dstResp2.setDestinationInfo(DestinationResponseBuilder.aDestinationResponse()
    ...
    public Mono<DestinationMapping> deleteDestinationMapping(String destMappingId) {
        return destinationMappingRepository.findById(Long.valueOf(destMappingId)).flatMap(destMappingEntity -> {
            return destinationMappingRepository.updateDestinationMappingStatus(destMappingEntity.getId(),
                    DestinationMappingStatus.DELETED)
                .onErrorStop()
                .then(destinationRepository.findById(destMappingEntity.getDestinationId()).flatMap(dest -> {
                    DestinationCacheEntity dce = DestinationCacheEntityBuilder.aDestinationCacheEntity()
                        ...
                        .build();
                    return destinationRedisRepository.deleteFromCache(destMappingEntity, dce).map(l -> {
                        logger.debug("{} destination mapping deleted from cache", l);
