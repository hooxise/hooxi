...
public class HooxiReactiveRedisConfiguration {
  ...
  @Bean
  ReactiveRedisOperations<String, DestinationCacheEntity> redisOperations(
      ReactiveRedisConnectionFactory factory,
      ObjectMapper mapper,
      RedisSerializer stringRedisSerializer) {
    Jackson2JsonRedisSerializer<DestinationCacheEntity> serializer =
        new Jackson2JsonRedisSerializer<>(DestinationCacheEntity.class);

    RedisSerializationContext.RedisSerializationContextBuilder<String, DestinationCacheEntity>
        builder =
            RedisSerializationContext.newSerializationContext(
                new DestinationRedisSerializer(mapper));

    RedisSerializationContext<String, DestinationCacheEntity> context =
        builder.key(stringRedisSerializer).value(serializer).build();
