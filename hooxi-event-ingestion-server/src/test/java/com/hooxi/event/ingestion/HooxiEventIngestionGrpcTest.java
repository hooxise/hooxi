...
/*@SpringBootTest(properties = {"grpc.server.inProcessName=hooxi-grpc", // Enable inProcess server
        "grpc.server.port=-1", // Disable external server
        "grpc.client.inProcess.address=in-process:hooxi-grpc" // Configure the client to connect to the inProcess server
})*/
...
public class  HooxiEventIngestionGrpcTest {
    ...
    static void createKeyspace(Cluster cluster) {
        try(Session session = cluster.connect()) {
            ...
            session.execute("CREATE TABLE IF NOT EXISTS test.hooxi_event (eventsource text, eventtype text, eventuri text, externaleventid text, internaleventid text, payload text, tenant text, timestamp text, status text, headers map<text,text>, createcloudevent boolean, PRIMARY KEY (internaleventid));");
        }
    }
     ...
     void createKeyspace() {
           ...
           // cqlTemplate.execute("CREATE KEYSPACE IF NOT EXISTS " + KEYSPACE_NAME + " WITH replication = \n" +
             //       "{'class':'SimpleStrategy','replication_factor':'1'};");
        //cqlTemplate.execute("CREATE TABLE IF NOT EXISTS .hooxi_event (eventsource text, eventtype text, eventuri text, externaleventid text, internaleventid text, payload text, tenant text, timestamp text, PRIMARY KEY (internaleventid));");
    ...
    public void testHooxiSendEvent() throws Exception {
        ...
        logger.debug("response received for event id {}, hooxiEventId {}", reply.getEventId(), reply.getHooxiEventId());
        assertThat(reply.getEventId()).isEqualTo("testEvent1");
        assertThat(reply.getHooxiEventId()).isNotEmpty();
        List<HooxiEventEntity> persistedHooxiEvents = hooxiEventRepository.findAll().collectList().block().stream().peek(System.out::println).toList();
